...
class DatasetTemplate(torch_data.Dataset):
    ...
    def collate_batch(batch_list, _unused=False):
        ...
        for key, val in data_dict.items():
            try:
                if key in ['voxels', 'voxel_num_points', 'coop_voxels', 'coop_voxel_num_points', 'coop_voxels_low', 'coop_voxel_num_points_low', 'coop_voxels_mid', 'coop_voxel_num_points_mid', 'coop_voxels_high', 'coop_voxel_num_points_high']:
                    if isinstance(val[0], list):
                        batch_size_ratio = len(val[0])
                        val = [i for item in val for i in item]
                    ret[key] = np.concatenate(val, axis=0)
                elif key in ['points', 'coop_points', 'voxel_coords', 'coop_voxel_coords', 'voxel_coords_low', 'coop_voxel_coords_low', 'voxel_coords_mid', 'coop_voxel_coords_mid', 'voxel_coords_high', 'coop_voxel_coords_high']:
                    coors = []
                    if isinstance(val[0], list):
                        val =  [i for item in val for i in item]
