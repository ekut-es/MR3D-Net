...
class Object3d_tb(BEVBox3D):
    ...
    def __init__(self, bounding_box):
        ...
        self.alpha = 0
        self.box2d = np.array([0, 0, 0, 0], dtype=np.float32)

        super().__init__(center, size, yaw, class_name, 1.0,  world_cam, cam_img)
...
class PCViewer:
    ...
    def draw_point_cloud(self):
        ...
        intensities = np.full_like(self.point_cloud[:, 3:].reshape(-1), 0.75)#1-(self.point_cloud[:, 3:].reshape(-1)/255)
        cm = plt.get_cmap('jet')
        ...
        #vis = open3d.visualization.ExternalVisualizer()
        #vis.set(pcd)
        vis.create_window()
