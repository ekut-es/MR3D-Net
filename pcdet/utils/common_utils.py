...
def check_nan_inf(out, id=None):
    if isinstance(out, int) or isinstance(out, float) or isinstance(out, np.ndarray):
        return
    if isinstance(out, SparseConvTensor):
        out = out.features
    nan_mask = torch.isnan(out)
    inf_mask = torch.isinf(out)
    if nan_mask.any():
        raise RuntimeError(f"Found NAN in output {id}")
    if inf_mask.any():
        raise RuntimeError(f"Found INF in output {id}")

def nan_hook(self, inp, output):
    if isinstance(output, torch.Tensor):
        check_nan_inf(output)

    elif isinstance(output, SparseConvTensor):
        check_nan_inf(output.features)

    elif isinstance(output, dict):
        for out_key, out_val in output.items():
            if out_key in ['calib', 'frame_id']:
                continue
            if isinstance(out_val, dict):
                for out_key_sub, out_val_sub in out_val.items():
                    check_nan_inf(out_val_sub, out_key_sub)
            else:
                check_nan_inf(out_val, out_key)
    elif isinstance(output, tuple):
        for item in output:
            if isinstance(item, dict):
                for out_key_sub, out_val_sub in item.items():
                    check_nan_inf(out_val_sub, out_key_sub)
            else:
                check_nan_inf(item)
    else:
...
def mask_points_by_range_with_z(points, limit_range):
    mask = (points[:, 0] >= limit_range[0]) & (points[:, 0] <= limit_range[3]) \
           & (points[:, 1] >= limit_range[1]) & (points[:, 1] <= limit_range[4]) \
