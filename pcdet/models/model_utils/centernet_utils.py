...
def _topk(scores, K=40):
    ...
    topk_ys = torch.div(topk_inds, width, rounding_mode='trunc').int()
    #assert torch.all(torch.eq(topk_ys_new, topk_ys))
    topk_xs = (topk_inds % width).int().float()

    topk_score, topk_ind = torch.topk(topk_scores.view(batch, -1), K)
    #topk_classes = (topk_ind // K).int()
    topk_classes = torch.div(topk_ind, K, rounding_mode='trunc').int()
    #assert torch.all(torch.eq(topk_classes_new, topk_classes))
...
def decode_bbox_from_pred_dicts(pred_dict, point_cloud_range=None, voxel_size=None, feature_map_stride=None):
    ...
    batch_dim = pred_dict['dim'].permute(0, 2, 3, 1).contiguous().view(batch_size, H*W, 3)  # (B, H, W, 3)
