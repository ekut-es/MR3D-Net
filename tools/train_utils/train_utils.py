...
def train_model(model, optimizer, train_loader, model_func, lr_scheduler, optim_cfg,
                ...
                use_logger_to_record=False, logger=None, logger_iter_interval=None, ckpt_save_time_interval=None, show_gpu_stat=False, cfg=None, args=None):
    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
        ...
        for cur_epoch in tbar:
            ...
            #-----------------------------------------------
            if cur_epoch >= args.eval_from_epoch:
                ...
                tb_dict = eval_utils.eval_one_epoch(cfg, args, model, test_loader, str(cur_epoch), logger, dist_test=False, result_dir=None)
                
                if cfg.LOCAL_RANK == 0:
                    for key, val in tb_dict.items():
                        tb_log.add_scalar(key, val, str(cur_epoch))
                if tb_dict.get('ap_70', None) is not None:
                    train.report({"AP70": tb_dict['ap_70']})

                logger.info('Epoch %s has been evaluated' % str(cur_epoch))
            #-----------------------------------------------
